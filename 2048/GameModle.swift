...
class GameModle: NSObject {
    ...
    //初始化一个都存的Empty的SequenceGameboard<TileEnum>
    init(dimension: Int, threshold: Int, delegate: GameModelProtocol) {
        self.dimension = dimension
        self.threshold = threshold
        self.delegate = delegate
        gamebord = SequenceGamebord(demission: dimension, initValue: TileEnum.Empty)
        super.init()
    }
    
    //--------move相关
    //提供给主控制器调用，入餐为移动方向和一个需要一个是否移动过的Bool值为入餐的毕包
    ...
    func performMove(direction: MoveDirection) -> Bool {
        ...
        for i in 0 ..< self.dimension {
            ...
            //对算法返回结果进行具体处理。1:更新gamebord中的数据，2:更新视图中的数据块
            for order in moveOrders {
                switch order {
                    //但个移动或合并的
                case let .SINGLEMOVEORDER(source: s, destination: d, value: v, merged: m):
                    ...
                    //两个进行合并的
                case let .DOUBLEMOVEORDER(firstSource: fs, secondSource: ts, destination: d, value: v):
    ...
    func insertRandomPositionTile(value: Int) {
        ...
        delegate.insertTile(pos: (x, y), value: value)
    }
    ...
    func condense(group: [TileEnum]) -> [TileAction] {
        ...
        for (index, tile) in group.enumerated() {
            switch tile {
                ...
                //下一个2时，buffer大小为1，groupindex为2，则需要移动
    ...
    func collapse(group: [TileAction]) -> [TileAction] {
        ...
        //是否跳过下一个，b如果把下一个块合并过来，则下一个数字块应该跳过
        var skipNext = false
        for (idx, token) in group.enumerated() {
            ...
            case let .NOACTION(source: s, value: v)
                where (idx < group.count - 1)
                    && v == group[idx + 1].getValue()
                    && quiescentTileStillQuiescent(inputPosition: idx, outputLength: tokenBuffer.count, originalPosition: s):
            ...
            case let .NOACTION(source: s, value: v) where !quiescentTileStillQuiescent(inputPosition: idx, outputLength: tokenBuffer.count, originalPosition: s):
    ...
    func convert(group: [TileAction]) -> [MoveOrder] {
        ...
        for (idx, tileAction) in group.enumerated() {
            ...
            case let .SINGLECOMBINE(source: s, value: v):
                //将一个块由s位置移动到idx位置，且idx位置由数字块，俩数字块进行合并，新值为v
                buffer.append(MoveOrder.SINGLEMOVEORDER(source: s, destination: idx, value: v, merged: true))
            case let .DOUBLECOMBINE(firstSource: s, secondSource: d, value: v):
